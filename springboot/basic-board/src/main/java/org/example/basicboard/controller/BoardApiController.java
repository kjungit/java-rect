...
/**
 * Swagger 어노테이션들
 * - @Tag : Controller(그룹) 단위의 설명 - 화면에서 API를 묶는 큰 제목이 된다.
 * - @Opration : 메서드 (APi 한 개) 단위의 설명 - 요약(Summary) / 상세(description)
 * - @Parameter : 파라미터 하나에 대한 설명
 * - @ApiResponse(s) : 이 API가 낼 수 있는 응답(상태코드별)을 문서에 명시
 * - @Content / @Schema : 응답/요청 본문의 "형태"를 지정
 */
...
public class BoardApiController {
    ...
    @ApiResponses(
            {
                    @ApiResponse(
                            responseCode = "200", description = "파일 다운로드",
                            content = @Content(
                    ...
                    @ApiResponse(
                            responseCode = "404", description = "해당 이름의 파일이 없음",
                            content = @Content(schema = @Schema(implementation = ErrorResponseDto.class))
                    )
            }
    )
