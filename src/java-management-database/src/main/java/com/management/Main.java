...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            printMenu(manager, pricePlan.getId());
            int menu = inputNumber("> ");
            switch (menu) {
                case 1:
                    addMember(manager, pricePlan.getId());
                    break;
                case 2:
                    findByEmail(manager, pricePlan.getId());
                    break;
                case 3:
                    findByName(manager, pricePlan.getId());
                    break;
                case 4:
                    manager.printAll(pricePlan.getId());
                    break;
                case 5:
                    updateMember(manager, pricePlan.getId());
                    break;
                case 6:
    ...
    private static void printMenu(MemberManager manager, int pricePlanId) {
        System.out.println();
        System.out.println("[수행할 업무 - 현재 회원수 : "
                + manager.getCount(pricePlanId) + "/" + manager.getCapacity() + "]");
        System.out.println("[1]회원추가 [2]회원조회(메일) [3]회원조회(이름)");
        System.out.println("[4]전체조회 [5]수정 [6]삭제 [7]종료");
    }

    private static void addMember(MemberManager manager, int pricePlanId) {
        if (manager.isFull(pricePlanId)) {
        ...
        if (grade == 1) {
            member = new Member(name, email, phone, Grade.NORMAL);
        } else {
            member = new Member(name, email, phone, Grade.VIP);
        }
    ...
    private static void findByEmail(MemberManager manager, int pricePlanId) {
        String email = inputText("조회할 이메일 > ");
        Member member = manager.findByEmail(email, pricePlanId);
    ...
    private static void findByName(MemberManager manager, int pricePlanId) {
        String name = inputText("조회할 이름 > ");
        Member member = manager.findByName(name, pricePlanId);
    ...
    private static void updateMember(MemberManager manager, int pricePlanId) {
        String targetEmail = inputText("수정할 회원의 이메일 > ");
        Member targetMember = manager.findByEmail(targetEmail, pricePlanId);
        ...
        boolean result = manager.update(targetEmail, newName, newEmail, newPhone, targetMember.getGrade(), pricePlanId);
